...
const AUTHORIZER_MAX_FACTS: u64 = 1000;
const AUTHORIZER_MAX_ITERATIONS: u64 = 100;
const AUTHORIZER_MAX_TIME: Duration = Duration::from_millis(20);
...
pub fn authorize(
    ...
    let mut builder = AuthorizerBuilder::new().set_limits(authorizer_limits());
...
pub enum VerifyError {
    #[error("biscuit error: {}", format_biscuit_token_error(.0))]
...
fn format_biscuit_token_error(error: &biscuit_auth::error::Token) -> String {
    match error {
        biscuit_auth::error::Token::RunLimit(limit) => {
            format!("{error}: {limit}")
        }
        _ => error.to_string(),
    }
...
mod tests {
    ...
    #[test]
    fn test_authorizer_limits_are_explicitly_configured() {
        let limits = authorizer_limits();

        assert_eq!(limits.max_facts, AUTHORIZER_MAX_FACTS);
        assert_eq!(limits.max_iterations, AUTHORIZER_MAX_ITERATIONS);
        assert_eq!(limits.max_time, AUTHORIZER_MAX_TIME);
        assert!(
            limits.max_time > Duration::from_millis(1),
            "authorization must not use biscuit-auth's 1ms default timeout"
        );
    }

    #[test]
    fn test_authorize_error_includes_inner_run_limit_reason() {
        let error = AuthorizeError::Biscuit(biscuit_auth::error::Token::RunLimit(
            biscuit_auth::error::RunLimit::Timeout,
        ));

        assert_eq!(
            error.to_string(),
            "biscuit error: Reached Datalog execution limits: spent too much time verifying"
        );
    }
    ...
    /// Test that a token created by @flickfyi/biscuit (TypeScript) can be decoded
    /// and its public_key fact extracted by the Rust biscuit-auth crate.
    ///
    /// This token was generated by:
    ///   PrivateKey.fromBase58("FAWSNU1cXqoCC4V5GY1q2pDnUDThHwrozVBSxw8kVMyQ", "secp256r1")
    /// with a public_key fact containing a random worker key.
    ///
    /// If this test fails, it means the TS biscuit library's protobuf/symbol encoding
    /// is incompatible with the Rust biscuit-auth crate's print_block_source.
    #[test]
    fn test_ts_generated_token_block_source_has_public_key_fact() {
        use base64ct::Encoding;

        use crate::auth::keys::RootKey;

        let root_key = RootKey::from_base58("FAWSNU1cXqoCC4V5GY1q2pDnUDThHwrozVBSxw8kVMyQ")
            .expect("should parse dev root key");
        let root_public_key = root_key.public_key();
        let biscuit_pubkey =
            public_key_to_biscuit(&root_public_key).expect("should convert root key");

        // Token generated by @flickfyi/biscuit with base64url encoding
        let token_b64 = "_50ptTZLYHghY1zs6RTV-E8IlvsZ3q81C_Vgm30lBpHMEUCIQCPfHPcDgW3avFTgbemAujyFOa_f3O9t1t2pEXkoSVsyAIgI6KIfwFtimpF-FW1Nn1gtA5Cy_7nG9z80SxJ-fnnxnkoASIiCiAOVNkz-FM1Gy-Sa0kHlBhDfRXEToUPvYoQKmVLdSDBfw==";

        // Decode base64url (with padding)
        let token_bytes =
            base64ct::Base64Url::decode_vec(token_b64).expect("should decode base64url token");

        // Parse with Rust biscuit-auth
        let biscuit = Biscuit::from(&token_bytes, biscuit_pubkey)
            .expect("should parse TS-generated biscuit token");

        // Print block source — this is what extract_client_public_keys uses
        let block_source = biscuit
            .print_block_source(0)
            .expect("should print block source");
        println!("TS token block source:\n{}", block_source);

        // The block source MUST contain public_key("...") for extraction to work
        assert!(
            block_source.contains("public_key(\""),
            "Block source from TS-generated token must contain public_key fact, got:\n{}",
            block_source
        );

        // Full extraction should work
        let keys = extract_client_public_keys(&biscuit)
            .expect("should extract public keys from TS-generated token");
        assert!(!keys.is_empty(), "Should extract at least one public key");
    }
}
